...
public class MainActivity extends AppCompatActivity {
    ...
    public Brewery getBrewery(String id) { return mCatalog.getBrewery(id); }
...
//                String query = URLEncoder.encode(urls[0],"UTF-8");
//                URL url = new URL(API_URL + "search?q=" + query + "&type=brewery&key=" + BREWERYDB_API_KEY);
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                try {
                    BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
                    StringBuilder stringBuilder = new StringBuilder();
    ...
    class RetrieveBeerListTask extends AsyncTask<String, Void, String> {
        ...
        protected String doInBackground(String... strings) {
            try {
                ...
                //URL url = new URL(API_URL + "brewery/" + URLEncoder.encode(strings[0], "UTF-8") + "/beers?key=" + BREWERYDB_API_KEY);
                HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
                try {
                    BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(urlConnection.getInputStream()));
                    StringBuilder stringBuilder = new StringBuilder();
        ...
        protected void onPostExecute(String response) {
            ...
            try {
                ...
                for (int i = 0; i < beerList.length(); i++) {
                    ...
                    mCatalog.addBeer(mBreweryID, new Beer(beer.getString("name")));
                }
